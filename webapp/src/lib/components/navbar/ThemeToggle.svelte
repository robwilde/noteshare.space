<script lang="ts">
	import DarkIcon from 'svelte-icons/io/IoMdMoon.svelte';
	import LightIcon from 'svelte-icons/io/IoMdSunny.svelte';

	let { dark = $bindable() }: { dark: boolean } = $props();

	function onClick() {
		dark = !dark;
	}
</script>

<button
	onclick={onClick}
	class="py-4 px-2 md:py-0 md:px-0 self-center w-full h-full text-zinc-600 dark:text-zinc-200 md:dark:text-zinc-400 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800 p-1.5"
>
	<span class="flex md:block justify-center items-center gap-2">
		<span class="whitespace-nowrap text-black dark:text-zinc-200 md:hidden">Toggle dark mode</span>
		<span class="h-full flex flex-col justify-center">
			<span class="inline-block w-6 h-6 text-black dark:text-zinc-300">
				{#if dark}
					<LightIcon />
				{:else}
					<DarkIcon />
				{/if}
			</span>
		</span>
	</span>
</button>
