...
const postRateLimit = rateLimit({
  ...
  max: parseInt(process.env.POST_LIMIT as string),
  standardHeaders: true,
  legacyHeaders: false,
});

const getRateLimit = rateLimit({
  windowMs: parseFloat(process.env.GET_LIMIT_WINDOW as string) * 1000,
  max: parseInt(process.env.GET_LIMIT as string),
